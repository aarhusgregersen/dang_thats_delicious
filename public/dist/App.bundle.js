...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    // Attach event listener to marker, that will display infoWindow
...
// TODO: Use geolocater to find user, then set as default
...
function typeAhead(search) {
  if (!search) return; // Just a simple error handler in case search isn't found or similar.
  ...
  searchInput.on('input', function () {
    ...
    // If there are no results; simply remove all
    if (!this.value) {
